...
ts.matrix.nls.lm.full <- function(x,orderx=c(1,0,0),ordery=c(1,0,0),h=1){
	...
	nls.out <- nls.lm(par=parStart, fn = residFun, observed = xl,
			xl = xl, control = nls.lm.control(nprint=1))
...
Method13_deBeer1985and1989.R <- function(ASFR,
	...
	########
	## if(is.na(sum(raw.data[,paste(c(year1,year2))])) | !any(apply(raw.cdata,2,function(x) all(!is.na(x)))) )
	## return(list(pop=pop,
	##            obsASFR=ASFR,
	...
	predCASFR[!is.na(predCASFR)&predCASFR < 0.00001] <- 0.00001
